...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Handle sub-menu toggling on mobile
    ...
    window.addEventListener('resize', function() {
        if (window.innerWidth >= 768) {
            ...
            // Remove any mobile-specific dropdown toggles
            const dropdownToggles = document.querySelectorAll('.dropdown-toggle');
            dropdownToggles.forEach(function(toggle) {
                toggle.remove();
            });
        } else {
            // Re-add dropdown toggles
            if (document.querySelectorAll('.dropdown-toggle').length === 0) {
                hasChildrenItems.forEach(function(item) {
                    const dropdownToggle = document.createElement('span');
                    dropdownToggle.className = 'dropdown-toggle';
                    dropdownToggle.innerHTML = '<i class="fas fa-chevron-down"></i>';
